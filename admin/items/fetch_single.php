...
if(isset($_POST["user_id"]))
{
	...
	foreach($result as $row)
	{
		...
		$output["popularitem"] = $row["popular_item"];
		if($row["name"] != '')
		{
			$output['user_image'] = '<img src="upload/'.$row["name"].'" class="img-thumbnail" width="50" height="35" /><input type="hidden" name="hidden_user_image" value="'.$row["name"].'" />';
		}
		else
		{
