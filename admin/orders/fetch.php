...
$query = '';
$output = array();
$query .= "SELECT orders.id, orders.customer_id, orders.total_price, orders.sub_total_price, orders.shipping_cost,orders.discount_amount,orders.payment_method,orders.delivery_man,orders.delivery_man_message,orders.admin_message,orders.created,orders.modified,orders.status,customer_registration.phone FROM orders JOIN customer_registration ON orders.customer_id=customer_registration.id ";
...
{
	$query .= 'WHERE customer_registration.phone LIKE "%'.$_POST["search"]["value"].'%" ';
	$query .= 'OR orders.id LIKE "%'.$_POST["search"]["value"].'%" ';
    $query .= 'OR orders.customer_id LIKE "%'.$_POST["search"]["value"].'%" ';	
}
